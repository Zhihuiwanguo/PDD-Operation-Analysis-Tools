...
# 兼容 Streamlit Cloud 在不同工作目录下执行脚本时的导入路径。
...
from app.pages import baibu_vs_normal, business_alerts, exceptions, links, overview, products, promotion, specs
    ...
    excel_blob = to_excel_bytes(
        {
            ...
            "推广-每日分析": ctx["promotion_analysis"].get("daily"),
            "推广-商品汇总": ctx["promotion_analysis"].get("goods"),
            "推广-明细": ctx["promotion_analysis"].get("detail"),
            **{f"推广异常-{k}": v for k, v in ctx["promotion_analysis"].get("anomalies", {}).items()},
