...
- 如线上已有 PR 发生冲突，建议先基于 **最新 main** 新建修复分支，再仅提交入口/部署说明相关文件。
- 这类修复建议尽量只包含：`streamlit_app.py`、`DEPLOY.md`，以及必要时的 `app/main.py`（导入兜底）。
...
### 报错 6：`ModuleNotFoundError: No module named 'app'`
**原因**：Cloud 运行时工作目录与本地不同，直接以 `app/main.py` 作为入口时，模块搜索路径可能不含仓库根目录。  
**处理**：
- 主文件路径改为：`streamlit_app.py`（推荐）
- 本项目已在 `app/main.py` 增加导入路径兜底逻辑。
